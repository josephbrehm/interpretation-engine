...
state = "CT" # has to be one at a time, due to the way the property data was generated

# aea
crs <- "+proj=aea +lat_1=29.5 +lat_2=45.5 +lat_0=37.5 +lon_0=-96 +x_0=0 +y_0=0 +ellps=GRS80 +datum=NAD83 +units=m +no_defs"

outname <- "CT300" # prefixed to all outputs 
res <- 300 # resolution for all the rasters
...
test <- dwb_calc2(as.data.frame(sf.nasis))
sf.nasis$dwb <- as.factor(test$dwb)
...
#plot(l.r$slope_r)
#plot(l.r$unstablefill) # should be awful but definitely text in the legend
#sapply(l.r, plot)

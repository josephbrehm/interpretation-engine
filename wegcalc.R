...
weg_calc <- function(datatable, # table with all data in it: output of weg_prep
  ...
  grtgrp <- tolower(datatable[row,"grtgrp"])
  compname <- tolower(datatable[row,"compname"])
  featkind2 <- tolower(datatable[row,"featkind2"])
  taxtemp <- tolower(datatable[row,"taxtemp"])
  taxord <- tolower(datatable[row,"taxord"])
  taxmin <- tolower(datatable[row,"taxmin"])
  
  # texture goes to uppercase though; shortcut taken to simplify translation from CVIR
  texture <- toupper(datatable[row,"texture"])

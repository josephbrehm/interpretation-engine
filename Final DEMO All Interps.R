...
#### HSG & SVI :
### tree_eval.r : Improve parallelization (part of tree_eval.r)
### Faster as raster math? Data trees as preferred structure didn't pan out for DWB and VF
### tree_eval.r : inherent raster mode (copy from calc_dwb code)
### tree_eval.r : rename to navigate_tree()
### svi_calc : write a function to do shorthsg if in a dataframe, or if a parameter set
### all : factoring is broken
### dwb : prune the list of layers
### all : demo layer substitution


#@@@@@@@@@@@@@@@@@@@@@@@@@@@#
#@@@@# USER INPUT HERE #@@@@#
#@@@@@@@@@@@@@@@@@@@@@@@@@@@#
## 0.1 set run parameters ### ###
...
wd <- "E:/NMSU/interp-engine" # set to interp engine github local
...
setwd(wd)
require(rasterVis)
require(sf) # used for spatial data
...
brick.svi$shorthsg <-   brick.svi$slope %>%
...
head(as.data.frame(out.dwb) %>% na.omit())
...
#levels(out.dwb$dwb) <- levels(out.dwb$dwb)[[1]][[1]][[2]]
out.dwb$dwb@data


levels(out.dwb$dwb) <- list(data.frame(
  ID = 1:3,
  VALUE = c("Not limited", "Somewhat limited", "Very limited")
))

levels(out.dwb$dwb) <- 

  factor(levels = rev(c("Not limited", "Somewhat limited", "Very limited")))




levelplot(d)
##### pseudo code for the ppt
load('brick.dwb.rdata')
names(brick.dwb)

out.dwb <- dwb_calc(brick.dwb) %>% mask(sf.studyarea)
plot(out.dwb$dwb)

newslope <- raster("E:/NMSU/Data/Slope/slope_pct_utGrand250.tif") %>% projectRaster(crs = crs)

brick2.dwb <- brick.dwb
brick2.dwb$slope_r <- newslope

out2.dwb <- dwb_calc(brick2.dwb) %>% mask(sf.studyarea)

...
# TO DO: 
# bug hunt. I am fairly certain the errors are in this function. Likely candidates are joins 3, 5, and 6
...
# updates:
# 7/28: Added option to output intermediate tables
...
weg_prep <- function(tbl.chorizon = ls.tables$chorizon,
  ...
  tbl.join <- 
    inner_join(tbl.chorizon, 
               tbl.component, by = "cokey") %>%
    inner_join(tbl.chtexturegrp, by = "chkey") %>%
    inner_join(tbl.chtexture, by = "chtgkey") %>%
    inner_join(tbl.mapunit, by = "mukey") %>%
    inner_join(tbl.legend, by = "lkey") %>%
    select(cokey, 
           compkind, 
           localphase, 
           taxgrtgroup, 
  ...
  # # AGGREGATE COLUMN taxminalogy FIRST.
    ...
    ls.SummaryTables[[3]] <- 
      inner_join(tbl.component, tbl.cotaxfmmin, by = "cokey") %>%
      inner_join(tbl.chorizon, by = "cokey") %>%
      mutate_if(is.factor, forcats::fct_explicit_na) %>%
      arrange(hzdept_r) %>%
      group_by(cokey) %>%
      summarise(.groups = "drop",
  ...
  ls.SummaryTables[[4]] <- 
    ...
    select(cokey, 
           featkind, 
           featdept_r) %>%  
    filter(tolower(featkind) == "andic soil properties" | is.na(featkind) | featkind == "(Missing)") %>%
    arrange(featdept_r) %>%
    group_by(cokey) %>%
    summarise(.groups = "drop",
  ...
  # ASSIGN rockfrags   	IF ISNULL(rockfrags) THEN 0 ELSE rockfrags.
   ...
   ls.SummaryTables[[5]] <- 
    ...
    filter(tolower(fragkind) != "wood fragments" | is.na(fragkind) | fragkind == "(Missing)") %>%  ### fragkind codes are different between nasis and gnatsgo
    filter(!is.na(fragvol_r)) %>%
    select(cokey, 
           chkey,
           fragvol_r, 
           hzdept_r,

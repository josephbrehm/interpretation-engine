...
source("E:/NMSU/interp-engine-personal/Functions/pull_SDA.r")
...
# 2 - flooding frequency   #### 
# this one goes further than the property, returning a T/F "does it flood" rather than freq 
...
l.i$lgstonepct <-
  ...
  mutate(fragvol_wmn = replace_na(fragvol_wmn), 0)
...
# 14 - unstable fill aka local phase  ####
...
indata.dwb <- l.i  %>% reduce(left_join, by = "cokey") %>% mutate_if(is.numeric, replace_na)
refdata.dwb <- l.t$dwb

#### SAVE IT!
save(indata.dwb, refdata.dwb, file = "indata.dwb.rda")

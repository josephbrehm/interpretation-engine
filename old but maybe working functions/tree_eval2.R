...
tree_eval <- function(
  ...
  out <- foreach(cell = 1:cells(indata),
                   .packages = "data.tree"
                   ) %dopar% {    
      ...
      while(!isLeaf(node))  { # when the node is a leaf (end node), the function is done
        ...
        ## NEW: GET ALL THE LOGICALS TOGETHER INTO A STRING. EVAL 1 FUN TO SEVERAL THINGS
        lstr.sibs <-
          sapply(1:length(parent$children), function(j) {parent[[j]]$logical}) %>%
          unlist() %>%
          collapse(
            paste("if(", 
            .),
...
hsg_eval <- function(indata, ncores = 1) {
  tree_eval(tree = tr.hsg,
            indata = indata,
            ncores = ncores)
}

svi_eval <- function(indata, ncores = 1) {
